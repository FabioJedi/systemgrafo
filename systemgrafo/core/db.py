...
from systemgrafo.contextmanager import Neo4jDBSessionManager
...
def wildcard_search(search_string):
    ...
    q = """
        OPTIONAL MATCH (m:Gene) WHERE m.title =~ {search_string} WITH collect(m) as genes
        OPTIONAL MATCH (p:Person) WHERE p.name =~ {search_string} WITH genes, collect(p) as persons
        RETURN genes, persons
        """
    with manager.session as s:
...
def get_actors(handle_id):
    q = """
        MATCH (n:Gene {handle_id: {handle_id}})<-[r:ACTED_IN]-(person)
...
def get_directors(handle_id):
    q = """
        MATCH (n:Gene {handle_id: {handle_id}})<-[r:DIRECTED]-(person)
...
def get_producers(handle_id):
    q = """
        MATCH (n:Gene {handle_id: {handle_id}})<-[r:PRODUCED]-(person)
        RETURN person.handle_id
        """
    with manager.session as s:
        result = s.run(q, {'handle_id': handle_id})
...
def get_writers(handle_id):
    q = """
        MATCH (n:Gene {handle_id: {handle_id}})<-[r:WROTE]-(person)
        RETURN person.handle_id
        """
    with manager.session as s:
        result = s.run(q, {'handle_id': handle_id})
...
def get_genes(handle_id):
    q = """
        MATCH (n:Person {handle_id: {handle_id}})-[r]->(gene)
        RETURN gene.handle_id, COLLECT(r) as relationships
        """
    with manager.session as s:
        result = s.run(q, {'handle_id': handle_id})
        for record in result:
